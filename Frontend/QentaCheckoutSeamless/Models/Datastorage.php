<?php
/**
 * QentaCheckoutSeamless Datastorage link
 *
 ...
 */
class Shopware_Plugins_Frontend_QentaCheckoutSeamless_Models_Datastorage
{
    /**
    ...
    /** @var QentaCEE_QMore_DataStorage_Response_Initiation $response */
     ...
     *
     * @return Shopware_Plugins_Frontend_QentaCheckoutSeamless_Models_Datastorage
     */
    public static function getSingleton()
    {
        if (!isset(self::$instance)) {
            self::$instance = new Shopware_Plugins_Frontend_QentaCheckoutSeamless_Models_Datastorage();
        }
        return self::$instance;
    }

    /**
     * Initiate QentaCheckoutSeamless datastorage
    ...
    public function initiate()
    {
        ...
        /** @var Shopware_Plugins_Frontend_QentaCheckoutSeamless_Models_Resources $seamless */
        $seamless = Shopware()->QentaCheckoutSeamless();
        /** @var Shopware_Plugins_Frontend_QentaCheckoutSeamless_Models_Config $config */
        $config = $seamless->Config();

        $dataStorageInit = new QentaCEE_QMore_DataStorageClient(Array(
        ...
        try {
            ...
            if ($this->response->getStatus() == QentaCEE_QMore_DataStorage_Response_Initiation::STATE_SUCCESS) {
                $dataStorageRead = new QentaCEE_QMore_DataStorage_Request_Read(Array(
                    'CUSTOMER_ID' => $config->CUSTOMERID,
                    'SHOP_ID'     => $config->SHOPID,
                    'SECRET'      => $config->SECRET,
                    'LANGUAGE'    => Shopware()->Locale()->getLanguage(),
        		...
        		foreach($dsErrors AS $error)
		        {
                    if (strcmp('undefined', strtolower($error))) {
                        Shopware()->QentaCheckoutSeamless()->qenta_action = 'undefined';
                    }
                    $msg[] = 'DataStorage: ' . $error->getConsumerMessage();
        		}
                Shopware()->QentaCheckoutSeamless()->qenta_action = 'error_init';
                Shopware()->QentaCheckoutSeamless()->qenta_message = implode(', ', $msg);
                Shopware()->Pluginlogger()->error('QentaCheckoutSeamless: '.__METHOD__ . ':' . implode(', ', $msg));
            }
        } catch (Exception $e) {
            if ($e instanceof QentaCEE_Stdlib_Exception_ExceptionInterface) {
    ...
    public function read()
    {
        ...
        try
        {
            return $dataStorageRead->read(Shopware()->QentaCheckoutSeamless()->storageId);
        } catch (Exception $e) {
            if ($e instanceof QentaCEE_Stdlib_Exception_ExceptionInterface) {
                Shopware()->Pluginlogger()->error('QentaCheckoutSeamless: DataStorage Read: ' . $e->getMessage());
    ...
    /**
     * Returns QentaCheckoutSeamless javascript url
     *
     * @return mixed
     */
