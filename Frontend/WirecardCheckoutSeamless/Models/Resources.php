 ...
 */
class Shopware_Plugins_Frontend_WirecardCheckoutSeamless_Models_Resources
{
   /**
    * Singleton pattern - only one instance of ourselves
    *
    * @var Shopware_Plugins_Frontend_WirecardCheckoutSeamless_Models_Resources
    */
    ...
    */
    public static function getSingleton()
    {
        if (!isset(self::$instance)) {
            self::$instance = new Shopware_Plugins_Frontend_WirecardCheckoutSeamless_Models_Resources();
            // Autoloader for lib
        }
    ...
    */
    public function __call($name = NULL, $arguments = NULL)
    {
        switch ($name) {
            case 'Loader':
                $name = 'Shopware_Plugins_Frontend_WirecardCheckoutSeamless_Components_' . $name;
                break;
            default:
                $name = 'Shopware_Plugins_Frontend_WirecardCheckoutSeamless_Models_' . $name;
                break;
        }
    ...
    {
    	Shopware()->Session()->_SESSION[Shopware_Plugins_Frontend_WirecardCheckoutSeamless_Bootstrap::NAME][$var] = serialize($val);
    }
	...
	{
		if (!empty($var) && isset(Shopware()->Session()->_SESSION[Shopware_Plugins_Frontend_WirecardCheckoutSeamless_Bootstrap::NAME][$var])) {
			return unserialize(Shopware()->Session()->_SESSION[Shopware_Plugins_Frontend_WirecardCheckoutSeamless_Bootstrap::NAME][$var]);
		}
		else {
			return NULL;
		}
    ...
    {
        $name = Shopware()->WirecardCheckoutSeamless()->getUser('payment')->name;
        return (is_null($name)) ? NULL : substr($name, strlen(Shopware()->WirecardCheckoutSeamless()->Config()->getPrefix('name')));
    }
