...
class WirecardCEE_QMore_ReturnFactory extends WirecardCEE_Stdlib_ReturnFactoryAbstract {
    ...
    public static function getInstance($return, $secret) {
        ...
        if (array_key_exists('paymentState', $return)) {
            ...
            throw new WirecardCEE_QMore_Exception_InvalidResponseException('Invalid response from Wirecard Checkout Seamless. Paymentstate is missing.');
        }
    }
    ...
    protected static function _getInstance($return, $secret) {
        switch(strtoupper($return['paymentState'])) {
            ...
            case parent::STATE_PENDING:
                ...
                break;
            default:
                throw new WirecardCEE_QMore_Exception_InvalidResponseException('Invalid response from Wirecard Checkout Seamless. Unexpected paymentState: ' . $return['paymentState']);
                break;
        }
    }
    ...
    protected static function _getSuccessInstance($return, $secret) {
        if (!array_key_exists('paymentType', $return)) {
            throw new WirecardCEE_QMore_Exception_InvalidResponseException('Invalid response from Wirecard Checkout Seamless. Paymenttype is missing.');
        }
